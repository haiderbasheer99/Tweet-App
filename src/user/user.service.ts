import { BadRequestException, ConflictException, forwardRef, HttpException, HttpStatus, Inject, Injectable, NotFoundException, RequestTimeoutException, UnauthorizedException } from '@nestjs/common';
...
export class UserService {
    ...
    public async createUser(userdto: CreateUserDto) {
        try {
            ...
            //create profile object and save it
    ...
    public async updateUserProfile(profileDto: UpdateProfileDto, userId: number) {
        try {
            ...
            if (!profile) {
                throw new NotFoundException(`there is no profile with this ${profileDto.id} id`)
            }
            ...
            profile.profileImage = profileDto.profileImage ?? profile.profileImage;
    ...
    public async emailVerification(user: Omit<User, 'password'>, otpType: OTPType) {
        try {
    ...
    public async findUserByUsername(username: string): Promise<User> {
        try {
            ...
            if (!user) {
                throw new NotFoundException(`User with this userName ${username} Does not Exist!`);
            }
    ...
    public async findUserWithEmail(email: string): Promise<Omit<User, 'password'>> {
        try {
