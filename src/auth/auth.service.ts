...
export class AuthService {
    ...
    public async login(logindto: LoginDto) {
        try {
            ...
            if (user.accountStatus === 'unverified') {
                if (!otp) {
                    return { message: 'Your account is not verified, please provide your otp..' }
                }
                else {
    ...
    async requestOtp(requestTokenDto: RequestTokenDto) {
        try {
            const userWithEmail = await this.userService.findUserWithEmail(requestTokenDto.email)
            if(!userWithEmail){
                throw new NotFoundException(`User With Email ${requestTokenDto.email} Not Found`);
            }
    ...
    private async generateToken(user: User) {
        ...
        const accessToken = await this.signToken<Partial<ActiveUserType>>(user.id, this.authConfig.expiresIn, { email: user.email })
